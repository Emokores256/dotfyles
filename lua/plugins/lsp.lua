	...
	},
	{
		...
		config = function()
			...
			vim.keymap.set(
				...
				"K",
				vim.lsp.buf.hover,
				{ desc = "Shows the documentation of what is under the cursor" }
			)
			vim.keymap.set("n", "gd", vim.lsp.buf.definition, { desc = "Shows definition" }, opts)
			vim.keymap.set({ "n", "v" }, "<leader>ca", vim.lsp.buf.code_action, { desc = "Code actions" }, opts)
			vim.keymap.set({ "n", "v" }, "<leader>l", vim.diagnostic.open_float, { desc = "Show line diagnostic" }, opts)
		end,
	},
}
